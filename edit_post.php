...
$update_post = $blogpost->getPost($id);
if (!$update_post) {
    header('Location: index.php');
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$errors) {
        ...
        if ($blogpost->updatePost($id, $post)) {
...
<head>
    ...
    <title>Pen Master | Edit Post</title>
</head>
    ...
    <main class="container pt-5">
        <form class="border rounded-3 p-5" action="#" method="post">
            ...
            <div class="form-group pb-3">
                ...
                <input type="text" class="form-control" name="title" value="<?= isset($_POST["title"]) ? $_POST["title"] : (isset($update_post["title"]) ? $update_post["title"] : "") ?>">
                <p><?= isset($errors["title"]) ? "enter title please" : "" ?></p>
            </div>
            <div class="form-group pb-3">
                <label class="text-capitalize" for="content">content:</label>
                <textarea name="content" class="form-control" id="content"><?= isset($_POST["content"]) ? $_POST["content"] : (isset($update_post["content"]) ? $update_post["content"] : "") ?></textarea>
